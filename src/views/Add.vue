    ...
    handleSubmit() {
      ...
      if (this.isFormValid) {
        if (this.inputContact.phoneNumbers)
          this.inputContact.phoneNumbers = this.inputContact.phoneNumbers.filter(
            (item) => item
          );
        ...
        if (this.inputContact.emails)
          this.inputContact.emails = this.inputContact.emails.filter(
            (item) => item
          );

        let emails = [];
        for (const email of this.inputContact.emails) {
          if (email.length > 0) emails.push(email.toLowerCase());
        }
